/*
 *
 *  * Designed and developed by 2020 codewithalaa (Alaa Alshammaa)
 *  *
...
class LoginRtlView extends StatefulWidget {
  ...
  static final FormFieldValidator<String> defaultPasswordValidator = (value) {
    if (value.isEmpty || value.length <= 2) {
      return 'كلمة مرور قصيرة جداً';
    }
    return null;
  };

  @override
  _LoginRtlViewState createState() => _LoginRtlViewState();
}

class _LoginRtlViewState extends State<LoginRtlView>
  ...
  Widget build(BuildContext context) {
    ...
    final emailValidator =
        widget.emailValidator ?? LoginRtlView.defaultEmailValidator;
    final passwordValidator =
        widget.passwordValidator ?? LoginRtlView.defaultPasswordValidator;

    final rtlLoginMessages = LoginMessages(
      usernameHint: 'البريد الإلكتروني',
      ...
      goBackButton: 'العودة',
      confirmPasswordError: 'غير متوافقة',
      recoverPasswordIntro: 'لاتحزن، تحصل في كافة الأوقات',
      recoverPasswordDescription:
          'إستعادة كلمة المرور الخاصة بحسابك في حال نسيانها :)',
      recoverPasswordSuccess: 'تم إستعادة كلمة المرور بنجاح',
    );
